    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(query, (err, rows) => {
            ...
            let triggers = (rows || []).map(tools.convertKeys).map(row => {
                ...
                let units = days * 24 * 3600 == row.seconds ? 'days':'hours';
                if (units == 'hours') {
                	days = Math.round(row.seconds / 3600);
                }
                row.formatted = util.format('%s %s after %s', days, units, row.rule === 'subscription' ? row.columnName : (util.format('%s <a href="/campaigns/view/%s">%s</a>', defaultEventMap[row.column], row.sourceCampaign, row.sourceCampaignName)));
...
module.exports.create = (trigger, callback) => {
    ...
    if (trigger.units == 'hours') {
    	seconds = (Number(trigger.days) || 0) * 3600;
    }
